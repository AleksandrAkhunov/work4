...
Сценарий: Реализация товара "Товар1" с количеством 5 шт
	...
	И я подготоваливаю вспомогательные Данные для проверки реализации
	...
	Тогда Остаток на складе корректно уменьшился. в регистре есть движение с видом расход, товаром Товар1 и Количеством 5
		Тогда таблица формы с именем "Список" стала равной:
			...
			| '5'          |  '1'            | 'Товар1' |
Сценарий: Подстановка поля "Акция" в реализация товара 
	Допустим я подготоваливаю вспомогательные Данные
	Когда я создаю новую расходную накладную
		Когда я закрыл все окна клиентского приложения
		И В панели разделов я выбираю "Продажа"
		И     В панели функций я выбираю "Продажа"
		Тогда открылось окно "Продажа"
		И     я нажимаю на кнопку "Создать"
		Тогда открылось окно "Продажа (создание)"
	Когда я меняю дату на 10.09.2016 
		Когда я нажимаю кнопку выбора у поля "Дата"
		И     в поле "Дата" я ввожу текст "10.09.2016  0:00:00"
	Тогда поле Акция должно быть равно "Продажа Атлант"
		Тогда элемент формы с именем "Акция" стал равен "Продажа Атлант"
